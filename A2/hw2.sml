...
fun all_except_option(s: string, sl: string list): string list option =
    let
        fun is_in_list(e, l): bool =
            case l
             of [] => false
              | e' :: l' => if same_string(e, e') then true
                    else is_in_list(e, l');
...
fun remove_card(cs: card list, c: card, e: exn): card list =
    ...
    in
        if is_in_list(c, cs) then safe_remove_card(cs, c) else raise e
    end;
...
fun officiate(deck: card list, moves: move list, goal: int): int =
    let
        fun helper_officiate
                (deck: card list, hand: card list, moves: move list) =
            if sum_cards(hand) > goal then score(hand, goal) else
            case moves
             of [] => score(hand, goal)
              | m :: moves' =>
                    case m
                     of Discard(c) => helper_officiate
                            (deck, remove_card(hand, c, IllegalMove), moves')
                      | Draw =>
                            case deck
                             of [] => score(hand, goal)
                              | c :: deck' => helper_officiate
                                    (deck', c :: hand, moves')
    in
        helper_officiate(deck, [], moves)
    end;
